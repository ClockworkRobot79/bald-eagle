...
router.get('/:restaurantID', (req, res) => {
    Restaurant.findById(req.params.restaurantID).populate({ path: 'menuItems', populate: {path: 'ratings', options: {sort: {'updatedAt': -1}}}}).populate({path: 'ratings', options: {sort: {'updatedAt': -1}}}).exec((err, restaurant) => {
        if (err) {
            console.error(`Error: ${err.message}`);
